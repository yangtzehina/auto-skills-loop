...
from openclaw_skill_create.models.runtime_governance import RuntimePriorRolloutReport
...
def _usage() -> str:
    return (
        'Usage: python scripts/run_runtime_prior_pilot.py '
        '[--format json|markdown] <prior_spec.json|rollout_report.json|->'
    )
...
def _load_payload(raw: str) -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        raise ValueError('Prior pilot input must be a JSON object')
    return payload
