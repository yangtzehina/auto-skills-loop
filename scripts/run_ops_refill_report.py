...
from openclaw_skill_create.services.ops_approval import DEFAULT_APPROVAL_MANIFEST, DEFAULT_COLLECTIONS_FILE, DEFAULT_OPS_ARTIFACT_ROOT, load_ops_approval_state
from openclaw_skill_create.services.ops_post_apply import build_ops_refill_report
...
def _usage() -> str:
    return (
        'Usage: python scripts/run_ops_refill_report.py '
...
def _parse_args(argv: list[str]) -> tuple[Path, Path, Path, Path, Path, Path, str]:
    ...
    return create_seed_source, prior_source, round_report, approval_manifest, artifact_root, collections_file, output_format


def main(argv: list[str]) -> int:
    try:
        create_seed_source, prior_source, round_report_path, approval_manifest, artifact_root, collections_file, output_format = _parse_args(argv)
