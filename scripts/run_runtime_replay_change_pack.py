...
def _usage() -> str:
    return (
        'Usage: python scripts/run_runtime_replay_change_pack.py '
...
def main(argv: list[str]) -> int:
    try:
        ...
        change_pack = build_runtime_replay_change_pack(**options)
    ...
    if output_format == 'markdown':
        print(change_pack.markdown_summary)
    else:
        print(json.dumps(change_pack.model_dump(mode='json'), indent=2, ensure_ascii=False))
    return 0 if change_pack.passed else 1
