...
from openclaw_skill_create.models.expert_studio import SkillProgramAuthoringPack  # noqa: E402
...
def _usage() -> str:
    return (
        'Usage: python scripts/run_skill_program_review.py '
...
def _load_pack(path: Path | None) -> SkillProgramAuthoringPack:
    if path is None:
        return build_skill_program_authoring_pack()
    ...
    return SkillProgramAuthoringPack.model_validate(payload)


def main(argv: list[str]) -> int:
    try:
        output_format, authoring_pack_path, output_root = _parse_args(argv)
        report = build_program_candidate_review_batch_report(_load_pack(authoring_pack_path))
    ...
    if output_root is not None:
        target = output_root / 'evals' / 'skill_program_review.json'
