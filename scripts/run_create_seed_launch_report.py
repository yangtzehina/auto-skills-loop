...
from openclaw_skill_create.services.ops_post_apply import build_create_seed_launch_report
from openclaw_skill_create.services.runtime_governance import build_runtime_create_seed_proposal_pack


def _usage() -> str:
    return (
        'Usage: python scripts/run_create_seed_launch_report.py '
        '[--candidate-key NAME] [--create-seed-source PATH] [--approval-manifest PATH] '
        '[--artifact-root PATH] [--output-root PATH] [--format json|markdown]'
    )


def _parse_args(argv: list[str]) -> tuple[str, Path, Path, Path, Path | None, str]:
    ...
    return candidate_key, create_seed_source, approval_manifest, artifact_root, output_root, output_format


def main(argv: list[str]) -> int:
    try:
        candidate_key, create_seed_source, approval_manifest, artifact_root, output_root, output_format = _parse_args(argv)
