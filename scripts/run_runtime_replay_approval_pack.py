...
def _usage() -> str:
    return (
        'Usage: python scripts/run_runtime_replay_approval_pack.py '
...
def main(argv: list[str]) -> int:
    try:
        ...
        approval = build_runtime_replay_approval_pack(**options)
    ...
    if output_format == 'markdown':
        print(approval.markdown_summary)
    else:
        print(json.dumps(approval.model_dump(mode='json'), indent=2, ensure_ascii=False))
    return 0 if approval.passed else 1
