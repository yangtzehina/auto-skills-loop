...
def _usage() -> str:
    return (
        'Usage: python scripts/run_runtime_prior_rollout.py '
        '[--format json|markdown] <prior_rollout_spec.json|->'
    )
...
def _load_spec(raw: str) -> dict[str, object]:
    ...
    if not isinstance(payload, dict):
        raise ValueError('Prior rollout spec must be a JSON object')
    return payload
...
def main(argv: list[str]) -> int:
    ...
    report = build_runtime_prior_rollout_report(
        ...
        runtime_effectiveness_lookup=dict(payload.get('runtime_effectiveness_lookup') or {}),
        rollout_min_runs=int(payload.get('rollout_min_runs', payload.get('runtime_effectiveness_min_runs', 5)) or 5),
    )
